...
```bash
node $PROJECT_ROOT/.claude-learning/tmp/ua-project-scan.js "$PROJECT_ROOT" "$PROJECT_ROOT/.claude-learning/tmp/ua-scan-results.json"
```
...
After the script completes, read `$PROJECT_ROOT/.claude-learning/tmp/ua-scan-results.json`. Do NOT re-run file discovery commands or re-count lines -- trust the script's results entirely.
...
1. Create the output directory: `mkdir -p <project-root>/.claude-learning/intermediate`
2. Write the JSON to: `<project-root>/.claude-learning/intermediate/scan-result.json`
