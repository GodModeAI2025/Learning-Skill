...
```bash
cat > $PROJECT_ROOT/.claude-learning/tmp/ua-arch-input.json << 'ENDJSON'
{
...
```bash
node $PROJECT_ROOT/.claude-learning/tmp/ua-arch-analyze.js $PROJECT_ROOT/.claude-learning/tmp/ua-arch-input.json $PROJECT_ROOT/.claude-learning/tmp/ua-arch-results.json
```
...
After the script completes, read `$PROJECT_ROOT/.claude-learning/tmp/ua-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the script's results entirely.
...
1. Write the JSON array to: `<project-root>/.claude-learning/intermediate/layers.json`
